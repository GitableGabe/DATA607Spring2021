...
Dataframe `manipulated` data is created using specifically columns `prof_id` and `score` from `evals` data set. Data is then condensed using `group_by()` function and a new column `no_rows` is added to the dataframe as shown below
...
Plotting with ggplot2 the plot type has to be chosen with additional functions such as `geom_line`,`geom_density`,`geom_histogram()`,`geom_point()`, etc. Multiple aesthetics can be applied in one graph as well, as shown by running `y=no_rows))+geom_histogram(aes(x=no_rows,..density..))+geom_density(aes(x=no_rows,..density..), color = "red", size=3)`
...
your console output should appear as \newline
...
$v \ Rendered \ reprex \ is \ on \ the \ clipboard.$ \newine
